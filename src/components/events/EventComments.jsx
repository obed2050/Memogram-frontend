...
const EventComments = ({ eventId }) => {
  ...
  const fetchComments = useCallback(async (pageNum = 1) => {
    try {
      setLoading(true);
      const res = await eventService.getComments(eventId, { page: pageNum, limit: 20 });
      if (pageNum === 1) {
  ...
  const handleCreate = async (e) => {
    ...
    try {
      setSubmitting(true);
      const res = await eventService.createComment(eventId, { content: content.trim() });
  ...
  const handleReply = async (e) => {
    ...
    try {
      setSubmitting(true);
      const res = await eventService.createReply(eventId, replyTo.id, { content: replyContent.trim() });
      setReplies((prev) => ({
        ...prev,
  ...
  const handleDeleteReply = async (replyId, parentCommentId) => {
    try {
      await eventService.deleteReply(replyId);
      setReplies((prev) => ({
        ...prev,
  ...
  const toggleReplies = async (comment) => {
    ...
    if (isExpanded) {
      ...
      if (!replies[comment.id]) {
        try {
          const res = await eventService.getReplies(eventId, comment.id, { limit: 50 });
    ...
    <div className="space-y-4">
      ...
      {loading && comments.length === 0 ? (
        ...
        <div className="text-center py-6">
          <HiChatBubbleLeftRight className="w-6 h-6 text-gray-600 mx-auto mb-1" />
