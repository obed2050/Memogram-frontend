...
import { communityService, generationService } from '../../services';
import toast from 'react-hot-toast';

const CommunityMembers = ({ schoolId, generation }) => {
  ...
  const fetchMembers = useCallback(async (pageNum = 1) => {
    try {
      const res = generation
        ? await generationService.getMembers(schoolId, generation, { page: pageNum, limit: 20 })
        : await communityService.getMembers(schoolId, { page: pageNum, limit: 20 });
          ...
          >
            ...
            <div className="min-w-0">
              ...
              <p className="text-xs text-gray-500 truncate">
                @{member.username}
                {!generation && member.generation && ` · ${member.generation}`}
                {generation && member.department && ` · ${member.department}`}
              </p>
            </div>
          </Link>
        ))}
